...
export function SheetControls({
  ...
  return (
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-4 sm:p-5 shadow-sm space-y-5">
      ...
      {/* Question Count & Time Allowed (2-Column Grid) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)] text-xs sm:text-sm text-[var(--color-text)] font-medium focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]"
          >
      ...
      {/* Output & Solutions Toggles (2-Column Clean Cards) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button
          type="button"
          ...
          className={`p-3 rounded-xl border text-left transition-all flex items-center justify-between cursor-pointer ${
            config.includeAnswerKey
              ...
              : 'bg-[var(--color-bg)] border-[var(--color-border)] text-[var(--color-text-muted)] hover:bg-slate-100 dark:hover:bg-slate-800'
          }`}
        >
          <div>
            <div className="text-xs font-bold">Solution Key Page</div>
            <div className="text-[10px] opacity-75">
              {config.includeAnswerKey ? '✓ Include Detached Key' : '✕ Exclude Solutions'}
            </div>
          </div>
          <span
          ...
          >
            {config.includeAnswerKey ? 'ON' : 'OFF'}
          </span>
        </button>
        ...
        <button
          ...
          className={`p-3 rounded-xl border text-left transition-all flex items-center justify-between cursor-pointer ${
            config.includeAnswerSpace
              ? 'bg-indigo-500/10 border-indigo-500/40 text-indigo-800 dark:text-indigo-200'
              : 'bg-[var(--color-bg)] border-[var(--color-border)] text-[var(--color-text-muted)] hover:bg-slate-100 dark:hover:bg-slate-800'
          }`}
        >
          <div>
            <div className="text-xs font-bold">Student Blank Lines</div>
            <div className="text-[10px] opacity-75">
              {config.includeAnswerSpace ? '✓ Print Working Lines' : '✕ No Blank Lines'}
            </div>
          </div>
          <span
            className={`text-xs px-2 py-0.5 rounded-full font-bold ${
          ...
          >
            {config.includeAnswerSpace ? 'ON' : 'OFF'}
          </span>
        </button>
      </div>
