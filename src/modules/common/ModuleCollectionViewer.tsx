...
import { Sparkles, Loader2, Search, SlidersHorizontal, BookOpen } from 'lucide-react';
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] text-sm text-[var(--color-text)] placeholder:text-[var(--color-text-subtle)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent transition-all shadow-xs"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text)] font-semibold"
                >
                  Clear
                </button>
              )}
            ...
            {/* Total items badge */}
            <div className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] text-xs text-[var(--color-text-muted)] shrink-0 self-start sm:self-auto">
              <BookOpen className="w-3.5 h-3.5 text-[var(--color-accent)]" />
              <span>
                <strong className="text-[var(--color-text)] font-semibold">{filteredItems.length}</strong> of {items.length} items
              </span>
            </div>
          </div>
          ...
          {filterOptions.length > 0 && (
            <div className="p-3.5 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] shadow-xs">
              <div className="flex items-center gap-2 mb-2 text-xs font-semibold text-[var(--color-text-muted)]">
        ...
        {activeExplanation && (
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="p-4 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] min-h-[180px] shadow-inner">
                {activeExplanation.text ? (
                  <MarkdownRenderer content={activeExplanation.text} />
                ) : (
                  <div className="flex flex-col items-center justify-center h-32 text-xs text-[var(--color-text-muted)] gap-2">
                    <Loader2 className="w-5 h-5 text-[var(--color-accent)] animate-spin" />
                    <span>Synthesizing authoritative step-by-step guidance...</span>
                  </div>
                )}
              </div>
            </div>
